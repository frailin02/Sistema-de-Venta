        ...
        public async Task<List<VentaDTO>> Historial(string buscarPor, string numeroVenta, string fechaInicio, string fechaFin)
        {
            ...
            try
            {
                if(buscarPor == "fecha")
                {
                    ...
                    DateTime fecha_Fin = DateTime.ParseExact(fechaInicio, "dd/MM/yyyy", new CultureInfo("es-DO"));
                    ListaResultado = await query.Where(v => v.FechaRegistro.Value.Date >= fecha_Inicio.Date &&
                    v.FechaRegistro.Value.Date <= fecha_Fin.Date).Include(dv => dv.DetalleVenta).ThenInclude(p => p.IdProductoNavigation).ToListAsync();
                }
                else
                {
