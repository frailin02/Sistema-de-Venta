    ...
    public class AutoMapperProfile: Profile
    {
        public AutoMapperProfile()
        {
            ...
            CreateMap<VentaDTO, Venta>()
                 .ForMember(destino =>
                destino.Total,
                opt => opt.MapFrom(origen => Convert.ToDecimal(origen.TotalTexto, new CultureInfo("es-DO")))
                );
            #endregion Venta
            #region DetalleVenta
            CreateMap<DetalleVenta, DetalleVentaDTO>()
                ...
                opt => opt.MapFrom(origen => Convert.ToString(origen.Precio.Value, new CultureInfo("es-DO"))))
                .ForMember(destino => destino.TotalTexto, opt => opt.MapFrom(origen => origen.Total.HasValue ? Convert.ToString(origen.Total.Value, new CultureInfo("es-DO")) : string.Empty));
            CreateMap<DetalleVentaDTO, DetalleVenta>()
                 .ForMember(destino =>
                destino.Precio,
                opt => opt.MapFrom(origen => Convert.ToDecimal(origen.PrecioTexto, new CultureInfo("es-DO")))
                )
                 .ForMember(destino =>
                destino.Total,
                opt => opt.MapFrom(origen => Convert.ToDecimal(origen.TotalTexto, new CultureInfo("es-DO")))
            ...
            CreateMap<DetalleVenta, ReporteDTO>()
                ...
                destino.Total, opt =>
                opt.MapFrom(origen => Convert.ToString(origen.Total.HasValue ? origen.Total.Value : 0, new CultureInfo("es-DO"))));
            #endregion Reporte


        }
    }
    //origen.Total.HasValue? Convert.ToString(origen.Total.Value, new CultureInfo("es-DO")) : string.Empty));
}
